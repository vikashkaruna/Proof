...
def layout(role: str) -> dict[str, tuple[Path, int, int]]:
    ...
    if role in ('issuer', 'runner'):
        result.update({
            ...
            'enroll.service': (Path('/etc/systemd/system') / f'axiom-spire-enroll-{role}.service', 0o644, 16384),
        })
    if role == 'runner':
        result.update({
            ...
            'controller_transition.py': (PREFIX / 'controller_transition.py', 0o600, 65536),
