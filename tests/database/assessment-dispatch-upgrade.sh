...
for path in Path('infra/supabase/migrations').glob('*.sql'):
    if int(path.name[:4]) < 44: shutil.copy2(path, Path(sys.argv[1])/path.name)
PY
docker exec "$container" createdb -U postgres dispatch_upgrade_test
sql() { docker exec -i "$container" psql -X -U postgres -d dispatch_upgrade_test -v ON_ERROR_STOP=1 -Atq "$@"; }
sql < tests/database/bootstrap.sql
python3 scripts/migrate-database.py --container "$container" --user postgres --database dispatch_upgrade_test --migrations "$fixture_dir" > "$fixture_dir/migrate.log"
sql <<'SQL'
...
before=$(fingerprint)
python3 scripts/migrate-database.py --container "$container" --user postgres --database dispatch_upgrade_test > "$fixture_dir/upgrade.log"
[ "$before" = "$(fingerprint)" ]
...
[ "$(sql -c 'select count(*) from public.assessment_dispatch_jobs')" = 0 ]
echo 'Dispatch upgrade: historical tasks/results preserved; no inferred queue, private payload or redispatch.'
