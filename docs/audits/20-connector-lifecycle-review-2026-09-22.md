...
- BFF/TS/Python: strict catalogue and role/tenant/request validation, error sanitization and separate interfaces. 364 BFF tests, 120 Python runtime tests and workspace TS/lint/typecheck/format pass before the committed acceptance lane. New Python files pass Ruff; an additional whole-runtime Ruff audit reports 65 pre-existing findings in unrelated files, outside the configured CI gate. The added YAML dependency was upgraded to patched 2.8.3 after audit flagged 2.8.2; production dependency audit then reports no known vulnerabilities.
...
The `05e265a` rerun passed the actual registration/retry/enable/disable/archive flow but still contained the old tenant-cookie assertion (the first text replacement missed it). An explicit patch corrected it; both focused connector journeys then passed against the running preprod containers. This is diagnosis only; full acceptance is rerun on the corrected committed suite.
