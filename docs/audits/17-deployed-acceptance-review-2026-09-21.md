...
Run 35636562142 exposed a Linux-only harness routing defect: HTTP API and SSR reads passed, but MFA/workbench and approval requests through the SSR→BFF bridge failed. SSR was addressing a BFF port published only on the host loopback interface through `host.docker.internal`; Docker Desktop allowed the route but Linux bridge networking did not. Application containers now share a private, per-run Docker network and address BFF by its network alias. Published browser/test ports remain loopback-only. Local bundled-Chromium CI-mode reproduction at 8c8b521 passed 118/118; a fresh Linux CI run is required for the network correction.
