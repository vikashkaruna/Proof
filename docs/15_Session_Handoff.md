...
Then migration 0026, which the review named as next: approval issuance in one transaction, with the content digest and eligibility rechecked under the row locks.

Then 0027, its other half. 0026 verified the digest when the approval was issued and nothing downstream checked it, so the token named which rows to execute and not what they contained. `trg_actions_approved_immutable` already freezes an approved action's parameters and rollback definition, but not its `dry_run_result` — the simulated outcome the approver actually read. `claim_plan_execution` now recomputes the digest under the action row locks and compares it with the value on the token, read from the persisted signed payload rather than the caller; a token with no snapshot is refused. The digest is signed into the approval token and carried onto the wire as dispatch contract v2. Details and the mutation results for both are in Doc 14.

Validation at this checkpoint: full 0000–0027 series on real PostgreSQL; nine SQL suites including the new `approval-issuance.test.sql` and `claim-snapshot.test.sql`; three concurrency suites; the DSN migration path and the self-hosted Supabase topology against real GoTrue and PostgREST; 255 BFF tests; 99 runtime tests; typecheck and lint 15/15; format; env-security, controls-drift and deployment-coverage gates. CI must also pass for the final staging head.

## Next implementation sequence

1. **W1 safety — the executor's half.** 0027 made the _claim_ enforce the snapshot: the digest is signed into the approval token, recomputed under the action row locks, and carried onto the wire as dispatch contract v2. What remains is the executor itself, which is a refusal stub and therefore records the snapshot rather than re-verifying against it. A real executor must recompute the digest against the rows it is about to mutate and refuse on a mismatch, proved with fault injection. Adding that check to the stub now would read as coverage while guarding nothing.
