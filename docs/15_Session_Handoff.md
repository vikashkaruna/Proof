...
3. **W1 remainder — deployment only.** The browser journeys are complete: refusal, offer, and an approval carried to a signed token, 47 green with nothing skipped. What is left is deployed acceptance. MFA key rotation is delivered, but the Cloud Run and Helm secret flows carry only the primary key, so a deployed rotation still has no path for the retiring list (an empty list cannot be a Secret Manager version, so it needs a conditional resource rather than another entry in the existing set), and no rotation has been exercised against a deployed environment.
