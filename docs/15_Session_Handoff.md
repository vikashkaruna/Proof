...
Logical checkpoint after delivering browser persona journeys against real authentication. The last verified code head before this checkpoint is `77c4dcd`. The merge commit containing this file is the resume baseline; fetch staging first. Current status: [Doc 11 Revision 16](11_Phase0-5_Gap_Closure_Plan.md), [progress](14_Implementation_Progress.md), [review 09](audits/09-reviewed-approval-snapshot-2026-09-21.md).

## Workspace and authorization

- Worktree: `/Users/vikash/Axiom Proof/.claude/worktrees/phase-0-5-gap-closure-5fd349`, branch `claude/phase-0-5-gap-closure-5fd349`, merged to staging with a merge commit. Preserve other worktrees and any uncommitted work in them.
...
3. **W1 remainder.** The persona journeys prove what each persona is _offered_; the positive case — an approver carrying an approval through to a signed token in a browser — needs the TOTP challenge satisfied against a running BFF, so the harness has to start the BFF as well as the web app. MFA key rotation is delivered, but the Cloud Run and Helm secret flows carry only the primary key, so a deployed rotation still has no path for the retiring list (an empty list cannot be a Secret Manager version, so it needs a conditional resource rather than another entry in the existing set), and no rotation has been exercised against a deployed environment.
