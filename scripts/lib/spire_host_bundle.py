...
def prepare(value: object, archive: bytes, ca: bytes | None = None) -> tuple[bytes, dict[str, bytes]]:
    ...
    for name in ('spire_host.py','spire_state.py','spire_enrollment.py') + (('spire_health.py','spire_volumes.py','controller_files.py','controller_placement.py','controller_runtime.py') if role=='runner' else ()):
